...
export function registerImportLeaderboards(server) {
    server.tool("import-leaderboards", "Import leaderboard configurations", {
        "inDir": z.string().describe("Input directory containing configurations"),
        ...
        try {
            let command = `ugs leaderboards import ${inDir} ${fileName}`;
            if (projectId) {
                command += ` -p ${projectId}`;
            }
            if (environmentName) {
