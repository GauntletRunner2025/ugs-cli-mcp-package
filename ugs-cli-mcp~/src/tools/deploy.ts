...
export function registerDeploy(server: McpServer) {
    server.tool(
        "deploy",
        "The Deploy command is an interface to deploy configuration files of supported services to the backend. Supported services: Remote Config, Cloud Code Scripts, Cloud Code Modules, Economy, Leaderboards, Access, Triggers, Scheduler and Matchmaker.",
        {
        ...
        async ({ projectId, environment, help, quiet, json, reconcile, dryRun, services, paths }) => {
            try {
                ...
                if (projectId) command += ` --project-id "${projectId}"`;
                if (environment) command += ` -e "${environment}"`;
                if (help) command += ` -h`;
                if (quiet) command += ` -q`;
                if (json) command += ` -j`;
                if (reconcile) command += ` --reconcile`;
                if (dryRun) command += ` --dry-run`;
                if (services) command += ` --services "${services}"`;
